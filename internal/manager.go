...
func NewManager(cfg *config.Config, isRestore bool) (*Manager, error) {
	...
	manager.MessageBuilder = NewSimplifiedMessageBuilder(manager)
...
// HasPreviousSession checks if there's a previous session that was restored
func (m *Manager) HasPreviousSession() bool {
	_, hasTitle := m.SessionOverrides["old_session_title"]
	return hasTitle
}

// GetPreviousSessionSummary generates an AI summary of the previous session
func (m *Manager) GetPreviousSessionSummary() string {
	if !m.HasPreviousSession() {
		return "No previous session data available."
	}
	
	title, _ := m.SessionOverrides["old_session_title"].(string)
	timestamp, _ := m.SessionOverrides["old_session_timestamp"].(time.Time)
	panes, _ := m.SessionOverrides["old_session_panes"].(map[string]string)
	files, _ := m.SessionOverrides["old_session_files"].([]string)
	
	// Simple summary format - in a real implementation, this would be generated by AI
	return fmt.Sprintf("Session '%s' (closed %s):\n- Session contained %d panes\n- Session had %d files in context\n- Last conversation had %d messages", 
		title, 
		time.Since(timestamp).Truncate(time.Minute).String()+" ago", 
		len(panes), 
		len(files),
		len(m.Messages))
}
